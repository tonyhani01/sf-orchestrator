...
Evolved from a private `orchestrator-mode` skill (same author; no third-party or internal-document content). Org-specific policy enters only via each user's own CLAUDE.md, which Claude Code delivers to subagents itself.
